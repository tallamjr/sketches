...
fn main() {
    ...
    let response_times = vec![
        10, 12, 15, 8, 20, 25, 30, 18, 22, 35,
        40, 45, 28, 32, 50, 60, 55, 42, 38, 65,
        70, 75, 80, 85, 90, 95, 100, 110, 120, 150,
        200, 180, 160, 140, 130, 125, 135, 145, 155, 175
    ];
    ...
    println!("Processed {} response time measurements", response_times.len());
    
    // Calculate key percentiles
    println!("\nResponse Time Percentiles:");
    println!("  50th percentile (median): {:.1}ms", kll.quantile(0.5).unwrap_or(0));
    println!("  90th percentile: {:.1}ms", kll.quantile(0.9).unwrap_or(0));
    println!("  95th percentile: {:.1}ms", kll.quantile(0.95).unwrap_or(0));
    println!("  99th percentile: {:.1}ms", kll.quantile(0.99).unwrap_or(0));
    
    // Test specific SLA thresholds
    let sla_threshold = 100;
    let rank_under_sla = kll.rank(&sla_threshold);
    println!("\nSLA Analysis:");
    println!("  Percentage under {}ms: {:.1}%", sla_threshold, rank_under_sla * 100.0);
    ...
    println!("\nQuantile Analysis:");
    println!("{:<10} {:<12} {:<12}", "Percentile", "Estimated", "Theoretical");
    println!("{}", "-".repeat(35));
    ...
    println!("East region - Median: {:.1}ms", 
             sketch_east.quantile(0.5).unwrap_or(0));
    println!("West region - Median: {:.1}ms", 
    ...
    println!("\nAfter merging:");
    println!("Global median: {:.1}ms", sketch_east.quantile(0.5).unwrap_or(0));
    println!("Global 95th percentile: {:.1}ms", sketch_east.quantile(0.95).unwrap_or(0));
    ...
    println!("{:<8} {:<12} {:<12} {:<10}", "k value", "Memory (KB)", "Error %", "Items");
    println!("{}", "-".repeat(45));
