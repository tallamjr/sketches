use sketches::{hll::HllSketch, theta::ThetaSketch, cpc::CpcSketch, bloom::BloomFilter};
...
fn batch_processing_demo() {
    ...
    println!("Both estimates: {:.0} vs {:.0}", hll.estimate(), hll_batch.estimate());
}
...
fn memory_efficiency_demo() {
    ...
    let sketches: Vec<HllSketch> = (0..10)
        .map(|_| HllSketch::new(12))
        .collect();
...
fn performance_comparison_demo() {
    ...
    println!("├─ HLL:   {:.2?} ({:.0} items/sec) - estimate: {:.0}", 
             hll_time, hll_rate, hll.estimate());
    println!("├─ Theta: {:.2?} ({:.0} items/sec) - estimate: {:.0}", 
             theta_time, theta_rate, theta.estimate());
    println!("├─ CPC:   {:.2?} ({:.0} items/sec) - estimate: {:.0}", 
             cpc_time, cpc_rate, cpc.estimate());
    println!("└─ Bloom: {:.2?} ({:.0} items/sec) - membership test ready", 
