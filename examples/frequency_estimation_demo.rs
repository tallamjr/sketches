...
fn main() {
    ...
    println!(
        "  Dimensions: {}x{} = {} cells",
        stats.width, stats.depth, stats.total_cells
    );
    ...
    println!(
        "{:<12} {:<10} {:<13} {:<10}",
        "Item", "Standard", "Conservative", "Difference"
    );
    println!("{}", "-".repeat(50));
    ...
    println!(
        "Calculated dimensions: {}x{}",
        stats_bounded.width, stats_bounded.depth
    );
    println!(
        "Theory: width ≈ e/ε ≈ {:.0}, depth ≈ ln(1/δ) ≈ {:.0}",
        std::f64::consts::E / 0.01,
        (1.0 / 0.01_f64).ln()
    );
