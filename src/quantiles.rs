    ...
    T: Clone + PartialOrd + std::fmt::Debug
{
    ...
    fn compact_level(&mut self, level: usize) {
        ...
        let items_to_promote: Vec<T> = self.levels[level].items
            .iter()
            .step_by(2)
            .cloned()
            .collect();
    ...
    pub fn quantile(&mut self, rank: f64) -> Option<T> {
        assert!(rank >= 0.0 && rank <= 1.0, "Rank must be between 0.0 and 1.0");
    ...
    pub fn merge(&mut self, other: &mut KllSketch<T>) {
        ...
        if let Some(other_min) = &other.min_value {
            match &self.min_value {
                ...
                Some(min) => if other_min.partial_cmp(min) == Some(Ordering::Less) {
        ...
        if let Some(other_max) = &other.max_value {
            match &self.max_value {
                ...
                Some(max) => if other_max.partial_cmp(max) == Some(Ordering::Greater) {
...
mod tests {
    ...
    fn test_kll_basic() {
        ...
        assert!((median as f64 - 500.0).abs() < 50.0, "Median should be around 500, got {}", median);
        
        let q95 = sketch.quantile(0.95).unwrap();
        assert!((q95 as f64 - 950.0).abs() < 50.0, "95th percentile should be around 950, got {}", q95);
        
        // Test rank
        let rank_500 = sketch.rank(&500);
        assert!((rank_500 - 0.5).abs() < 0.1, "Rank of 500 should be around 0.5, got {}", rank_500);
    }
    ...
    fn test_kll_merge() {
        ...
        assert!((median as f64 - 500.0).abs() < 100.0, "Merged median should be around 500");
    }
    ...
    fn test_kll_accuracy() {
        ...
        for &q in &quantiles {
            ...
            assert!(error < 0.3, "Quantile {} error {} too high (estimated: {}, expected: {})", 
                    q, error, estimated, expected);
        }
        
        // Test that median is reasonable
        let median = sketch.quantile(0.5).unwrap() as f64;
        assert!(median > 3000.0 && median < 7000.0, "Median {} should be roughly in middle range", median);
    }
