// CPC entropy-compression tables.
//
// The three ENCODING tables are copied verbatim from Apache DataSketches
// (lib/datasketches-rust/datasketches/src/cpc/compression_data.rs). The
// corresponding DECODING tables and inverse column permutations are GENERATED at
// runtime from the encoding tables via `make_decoding_table` and
// `make_inverse_permutation`, and cached behind `OnceLock`. The validation tests
// prove the generated decoders exactly invert the copied encoders, which also
// proves the encoding tables were copied without error.
//
// The encoding statics and the decoding-table accessors are the compression
// engine's inputs; they are consumed by the CPC (de)compressor added in later
// tasks of this sub-project. Until then they carry a narrowly-scoped `dead_code`
// allow (the validation tests exercise every accessor) rather than suppressing
// genuine errors elsewhere.
#![allow(dead_code)]

use std::sync::OnceLock;

#[rustfmt::skip]
pub(crate) static LENGTH_LIMITED_UNARY_ENCODING_TABLE65: [u16; 65] = [
...
#[rustfmt::skip]
pub(crate) static COLUMN_PERMUTATIONS_FOR_ENCODING: [[u8; 56]; 16] = [
    // for phase = 1 / 32
    [
...
#[rustfmt::skip]
pub(crate) static ENCODING_TABLES_FOR_HIGH_ENTROPY_BYTE: [[u16; 256]; 22] = [
...
/// Build a 4096-entry decoding table from an encoding table.
///
/// Each encoding entry packs a `(code_length << 12) | code_value`. The decoding
/// table is indexed by the next 12 bits of the input stream; every 12-bit
/// pattern whose low `code_length` bits equal `code_value` decodes to the same
/// byte, so we fill all `2^(12 - code_length)` such slots with the same
/// `(code_length << 8) | byte_value` entry.
fn make_decoding_table(encoding_table: &[u16], num_byte_values: u16) -> Vec<u16> {
    ...
    let mut decoding_table = vec![0u16; 4096];
    for byte_value in 0..num_byte_values {
        ...
        let num_copies = 1u16 << garbage_length;
...
/// Build the inverse of a permutation: `inverse[perm[i]] = i`.
...
/// Decoding table for the length-limited unary code (65 symbols).
pub(crate) fn length_limited_unary_decoding_table() -> &'static [u16; 4096] {
    static TABLE: OnceLock<[u16; 4096]> = OnceLock::new();
    TABLE.get_or_init(|| {
        let v = make_decoding_table(&LENGTH_LIMITED_UNARY_ENCODING_TABLE65, 65);
        v.try_into().expect("decoding table must have 4096 entries")
    })
}

/// The 22 decoding tables for the high-entropy byte (each 4096 entries).
pub(crate) fn high_entropy_decoding_tables() -> &'static [[u16; 4096]; 22] {
    static TABLES: OnceLock<Box<[[u16; 4096]; 22]>> = OnceLock::new();
    TABLES.get_or_init(|| {
        let mut tables: Box<[[u16; 4096]; 22]> = Box::new([[0u16; 4096]; 22]);
        for i in 0..22 {
            let v = make_decoding_table(&ENCODING_TABLES_FOR_HIGH_ENTROPY_BYTE[i], 256);
            tables[i] = v.try_into().expect("decoding table must have 4096 entries");
        }
        tables
    })
}

/// The 16 inverse column permutations used when decoding.
pub(crate) fn column_permutations_for_decoding() -> &'static [[u8; 56]; 16] {
    static PERMS: OnceLock<[[u8; 56]; 16]> = OnceLock::new();
    PERMS.get_or_init(|| {
        let mut perms = [[0u8; 56]; 16];
        for i in 0..16 {
            let v = make_inverse_permutation(&COLUMN_PERMUTATIONS_FOR_ENCODING[i], 56);
            perms[i] = v.try_into().expect("permutation must have 56 entries");
        }
        perms
    })
}
...
mod tests {
    use super::*;
    #[test]
    fn generated_decoding_tables_invert_encoding() {
        // length-limited unary
        validate(
            length_limited_unary_decoding_table(),
    ...
    #[test]
    fn inverse_permutations_round_trip() {
        let dec = column_permutations_for_decoding();
        for p in 0..16 {
            for i in 0..56u8 {
                assert_eq!(
                    COLUMN_PERMUTATIONS_FOR_ENCODING[p][dec[p][i as usize] as usize],
                    i
                );
            }
        }
